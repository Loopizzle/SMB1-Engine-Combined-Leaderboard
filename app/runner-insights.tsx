...
import { insightBoardLabel, nextTargets, runnerArchetype, runnerRivalries, type InsightBoard, type InsightPlayer, type InsightRun } from './insights';
...
export default function RunnerInsights({ player, players, runs, boards, gameNames, openProfile }: { player: InsightPlayer; players: InsightPlayer[]; runs: InsightRun[]; boards: InsightBoard[]; gameNames: Record<string, string>; openProfile: (runner: string) => void }) {
  ...
  return <section className="profile-section profile-intelligence"><h4>Runner intelligence</h4><div className="profile-archetype"><Sparkles size={18} /><span><small>Archetype</small><strong>{archetype.name}</strong><em>{archetype.detail}</em></span></div><div className="profile-insight-block"><h5><Network size={15} /> Closest rivals</h5>{rivals.map((rival) => <button key={rival.runner} onClick={() => openProfile(rival.runner)}><span><strong>{rival.runner}</strong><small>{rival.shared} shared boards · you lead {rival.wins}, trail {rival.losses}</small></span><em>#{rival.rank}</em></button>)}</div><div className="profile-insight-block"><h5><Target size={15} /> Timed score goals</h5>{targets.map((target) => <div key={target.board.boardKey}><span><strong>{gameNames[target.board.gameAbbr] || target.board.gameAbbr} - {insightBoardLabel(target.board)}</strong><small>{target.goalLabel}: beat {target.goalTime} for about #{target.proposedPlace}</small></span><em>+{fmt(target.estimatedGain)}</em></div>)}</div></section>;
}
